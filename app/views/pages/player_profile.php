...
$snapshotSeries = [$playTimeHours, $mined, $placed, $fishCaught, $kills, $deaths];
$snapshotMax = 1.0;
foreach ($snapshotSeries as $snapshotValue) {
    $snapshotMax = max($snapshotMax, (float)$snapshotValue);
}
$snapshotPoints = [];
$snapshotDots = [];
$snapshotCount = count($snapshotSeries);
foreach ($snapshotSeries as $index => $snapshotValue) {
    $x = 4 + (($snapshotCount > 1 ? 92 / ($snapshotCount - 1) : 0) * $index);
    $y = 38 - (($snapshotMax > 0 ? ((float)$snapshotValue / $snapshotMax) : 0) * 24);
    $snapshotPoints[] = number_format($x, 2, '.', '') . ' ' . number_format($y, 2, '.', '');
    $snapshotDots[] = [
        'x' => number_format($x, 2, '.', ''),
        'y' => number_format($y, 2, '.', ''),
    ];
}
$snapshotLinePath = $snapshotPoints !== [] ? 'M ' . implode(' L ', $snapshotPoints) : 'M 4 38 L 96 38';
$snapshotAreaPath = $snapshotPoints !== [] ? $snapshotLinePath . ' L 96 38 L 4 38 Z' : 'M 4 38 L 96 38 L 96 38 L 4 38 Z';
$staticTabs = ['总览', '近期动态', '偏好雷达', '成就收藏'];
...
.player-snapshot-chart svg {
    display: block;
    width: 100%;
    height: auto;
}

.player-snapshot-caption {
    margin-top: 0.65rem;
    color: var(--player-text-muted);
    font-size: 0.82rem;
}

.player-snapshot-gridline {
    stroke: var(--player-card-border);
    stroke-width: 0.45;
}

.player-snapshot-area {
    fill: rgba(14, 165, 233, 0.13);
}

.player-snapshot-line {
    fill: none;
...
.player-snapshot-dot {
    ...
    stroke: var(--player-card-bg-strong);
    stroke-width: 1.2;
}
    ...
    <div class="player-dashboard">
        <aside class="player-profile-card">
            ...
            <div class="player-action-row">
                <button type="button" class="player-action-btn is-active" id="player-skin-action">行动动画</button>
                <button type="button" class="player-action-btn" id="player-skin-run">跑步动画</button>
            </div>
        ...
        <section class="player-main">
            <header class="player-hero">
                <div>
                    ...
                    <div class="player-tab-row" aria-label="玩家分区标签">
                        <?php foreach ($staticTabs as $tabIndex => $staticTab): ?>
                            <span class="player-tab-pill<?= $tabIndex === 0 ? ' is-active' : '' ?>"><?= htmlspecialchars($staticTab, ENT_QUOTES, 'UTF-8') ?></span>
                        <?php endforeach; ?>
                    </div>
                </div>
            ...
            <div class="player-main-grid">
                <section class="player-panel player-panel--overview">
                    <p class="player-panel-kicker">总览</p>
                    <h3>综合数据统计</h3>
                    <p>优先展示已接入的真实字段，其余位置使用清晰的待同步提示。下方缩略图是字段归一化快照，不表示时间序列走势。</p>
                    ...
                    <div class="player-snapshot">
                        <div class="player-snapshot-chart" aria-hidden="true">
                            <svg viewBox="0 0 100 44" preserveAspectRatio="none" role="img">
                                <line class="player-snapshot-gridline" x1="4" y1="10" x2="96" y2="10"></line>
                                <line class="player-snapshot-gridline" x1="4" y1="24" x2="96" y2="24"></line>
                                <line class="player-snapshot-gridline" x1="4" y1="38" x2="96" y2="38"></line>
                                <path class="player-snapshot-area" d="<?= htmlspecialchars($snapshotAreaPath, ENT_QUOTES, 'UTF-8') ?>"></path>
                                <path class="player-snapshot-line" d="<?= htmlspecialchars($snapshotLinePath, ENT_QUOTES, 'UTF-8') ?>"></path>
                                <?php foreach ($snapshotDots as $snapshotDot): ?>
                                    <circle class="player-snapshot-dot" cx="<?= htmlspecialchars((string)$snapshotDot['x'], ENT_QUOTES, 'UTF-8') ?>" cy="<?= htmlspecialchars((string)$snapshotDot['y'], ENT_QUOTES, 'UTF-8') ?>" r="1.45"></circle>
                                <?php endforeach; ?>
                            </svg>
                        </div>
                        <div class="player-snapshot-caption">字段缩略图覆盖：游戏时长、挖掘、建造、钓鱼、击杀、死亡。后续接入更多真实统计后会自动更丰富。</div>
                    </div>
                </section>
                ...
                <section class="player-panel player-panel--achievements">
